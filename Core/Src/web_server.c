...
static void http_server_serve(struct netconn *conn) // new connection service
{
	...
	if (recv_err == ERR_OK)
	{
		if (netconn_err(conn) == ERR_OK)
		{
			...
			if ((buflen >=5) && (strncmp(buf, "GET /", 5) == 0)) // Is this an HTTP GET command? (only check the first 5 chars, since there are other formats for GET, and we're keeping it very simple )
			{
